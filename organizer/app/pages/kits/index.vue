    ...
    <template v-else>
      ...
      <template v-else-if="kits">
        ...
        <template v-else>
            ...
            <div class="border-b border-slate-100 px-5 py-4 flex flex-wrap items-center justify-between gap-2">
              <div>
                <p class="text-xs font-black uppercase tracking-wider text-slate-700">
                  Total de Camisetas a Produzir — {{ kits.totalComCamisa ?? kits.total }} unidades
                </p>
                <p v-if="kits.totalSemCamisa" class="text-[11px] text-slate-500 font-normal mt-0.5">
                  ({{ kits.totalSemCamisa }} atletas optaram por inscrição sem camisa)
                </p>
              </div>
          ...
          <!-- Por Modelo de Camisa (se houver modelos cadastrados) -->
          <div v-if="kits.porModelo && kits.porModelo.length > 0" class="space-y-3 pt-2">
            <h3 class="text-xs font-black uppercase tracking-wider text-slate-600 flex items-center gap-1.5">
              <Shirt :size="16" class="text-orange-600" />
              Demanda por Modelo de Camisa
            </h3>
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div
                v-for="modelo in kits.porModelo"
                :key="modelo.modeloId"
                class="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs space-y-3"
              >
                <div class="flex items-center justify-between">
                  <p class="font-bold text-sm text-slate-900">{{ modelo.modeloNome }}</p>
                  <span class="text-xs font-bold bg-orange-100 text-orange-800 px-2.5 py-0.5 rounded-full">
                    {{ modelo.total }} unidades
                  </span>
                </div>
                <div class="flex flex-wrap gap-3">
                  <div
                    v-for="[tamanho, quantidade] in ordenarTamanhos(modelo.tamanhos)"
                    :key="tamanho"
                    class="text-center bg-slate-50 px-3 py-2 rounded-lg border border-slate-100"
                  >
